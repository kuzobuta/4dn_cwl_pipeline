#!/bin/bash
#Modified from original "run-cool2multirescool.sh" by Hisashi
#ver. 230123
shopt -s extglob
ncores=1
chunksize=10000000
...
balance=1
d=2            #Added by Hisashi [number of ignoring diagonal]
outprefix=out
...
printHelpAndExit() {
    ...
    echo "-B : no balance"
    echo "-d : number of ignoring diagonal : default 2" #Added by Hisashi
    exit "$1"
}

while getopts "i:o:p:c:ju:B:d:" opt; do
    case $opt in
        i) input=$OPTARG;;
        o) outprefix=$OPTARG;;
        p) ncores=$OPTARG;;
...
if [[ $balance == "1" ]]; then
    balance_args=\'"--convergence-policy store_nan --ignore-diags $d"\' #Added by Hisashi
    cmd="cooler zoomify --balance --balance-args $balance_args -n $ncores -o $outprefix.mcool -c $chunksize $RES_ARG $input"
    eval ${cmd}
else
